...
const conexao = () => __awaiter(void 0, void 0, void 0, function* () {
    ...
    if (process.env.Node_env == "test" && conexao_string == localdb) {
        throw console.error(`$ ❌ {process.env.Node_env} - Falta de string de conexão ou utilização de banco de dados no ambiente de produção`);
    ...
    const mongodb = yield mongoose_1.default.connect(conexao_string, () => {
        console.log(`✔ - Mongo DB conectado com sucesso`); /* Conecta ao Mongo DB e printa um log na tela */
    });
});
